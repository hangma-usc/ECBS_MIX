...
namespace boost {
	...
	void my_successive_shortest_path_nonnegative_weights(
		const Graph &g,
		...
		filtered_graph<const Graph, is_residual_edge<ResidualCapacity> >
			gres = residual_graph(g, residual_capacity);
		...
		while (true) {
			...
			my_augment(g, s, t, pred, residual_capacity, rev);
		}
	}
	...
	namespace detail {
		...
		void my_successive_shortest_path_nonnegative_weights_dispatch3(
			const Graph &g,
			typename graph_traits<Graph>::vertex_descriptor s,
			typename graph_traits<Graph>::vertex_descriptor t,
			Capacity capacity,
			...
			Distance dist,
			Distance2 dist_pred) {
			my_successive_shortest_path_nonnegative_weights(g, s, t, capacity, residual_capacity, weight, rev, index, pred, dist, dist_pred);
		}
		...
		void my_successive_shortest_path_nonnegative_weights_dispatch3(
			...
			my_successive_shortest_path_nonnegative_weights(g, s, t, capacity, residual_capacity, weight, rev, index, pred, dist,
		...
		void my_successive_shortest_path_nonnegative_weights_dispatch2(
			Graph &g,
			typename graph_traits<Graph>::vertex_descriptor s,
			typename graph_traits<Graph>::vertex_descriptor t,
			Capacity capacity,
			...
			my_successive_shortest_path_nonnegative_weights_dispatch3(g, s, t, capacity, residual_capacity, weight, rev, index, pred, dist, get_param(params, vertex_distance2));
		}
		...
		void my_successive_shortest_path_nonnegative_weights_dispatch2(
			...
			my_successive_shortest_path_nonnegative_weights_dispatch3(g, s, t, capacity, residual_capacity, weight, rev, index, pred,
				make_iterator_property_map(d_map.begin(), index),
				get_param(params, vertex_distance2));
		}

		template <class Graph, class P, class T, class R, class Capacity, class ResidualCapacity, class Weight, class Reversed, class Pred, class VertexIndex>
		void my_successive_shortest_path_nonnegative_weights_dispatch1(
			Graph &g,
			typename graph_traits<Graph>::vertex_descriptor s,
			typename graph_traits<Graph>::vertex_descriptor t,
			Capacity capacity,
			...
			Pred pred,
			const bgl_named_params<P, T, R>& params) {
			my_successive_shortest_path_nonnegative_weights_dispatch2(g, s, t, capacity, residual_capacity, weight, rev, index, pred,
		...
		void my_successive_shortest_path_nonnegative_weights_dispatch1(
			...
			my_successive_shortest_path_nonnegative_weights_dispatch2(g, s, t, capacity, residual_capacity, weight, rev, index,
		...
		void my_successive_shortest_path_nonnegative_weights(
			...
			return detail::my_successive_shortest_path_nonnegative_weights_dispatch1(g, s, t,
		...
		void my_successive_shortest_path_nonnegative_weights(
			...
			my_successive_shortest_path_nonnegative_weights(g, s, t, params);
		}

		// Keep residual edges (used in maximum-flow algorithms).
		template <typename ResidualCapacityEdgeMap>
		struct my_is_residual_edge {
			my_is_residual_edge() { }
			my_is_residual_edge(ResidualCapacityEdgeMap rcap) : m_rcap(rcap) { }
		...
		filtered_graph<const Graph, my_is_residual_edge<ResCapMap> >
			residual_graph(const Graph& g, ResCapMap residual_capacity) {
			return filtered_graph<const Graph, my_is_residual_edge<ResCapMap> >
				(g, my_is_residual_edge<ResCapMap>(residual_capacity));
		}

		template <class Graph, class PredEdgeMap, class ResCapMap,
		class RevEdgeMap>
			inline void
			my_augment(const Graph& g,
				...
				RevEdgeMap reverse_edge)
		{
			...
			e = get(p, sink);
			do {
				residual_capacity[e] = residual_capacity[e] - 1;
				residual_capacity[get(reverse_edge, e)] = residual_capacity[get(reverse_edge, e)] + 1;
				u = source(e, g);
				e = get(p, u);
			} while (u != src);
		}
