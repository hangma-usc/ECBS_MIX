...
SingleTypeSearchECBS::SingleTypeSearchECBS(vector<int> s_locs, vector<int> g_locs, const double* heuristic,
...
long SingleTypeSearchECBS::getVertexIndex(VertexProperties* v_pt) {
	return (long)((((v_pt->loc1 * map_size + v_pt->loc2) * 10000 + v_pt->timestep) * 2 + v_pt->is_in_node) * 2 + v_pt->is_out_node) * 2 + v_pt->is_u_node;
}

inline bool SingleTypeSearchECBS::existVertex(Graph_t & g, VertexProperties* v_pt) {
...
SingleTypeSearchECBS::Graph_Vertex_t SingleTypeSearchECBS::getVertex(Graph_t & g, VertexProperties* v_pt) {
...
SingleTypeSearchECBS::Graph_Vertex_t SingleTypeSearchECBS::addVertex(Graph_t & g, VertexProperties* v_pt) {
	Graph_Vertex_t v = boost::add_vertex(g);
	g[v] = *v_pt;
	vertex_index_map[v_pt] = v;
	return v;
...
inline bool SingleTypeSearchECBS::isDirectionAllowed(int location, int direction) {
...
void SingleTypeSearchECBS::setUntillLayer(int layer_time_step) {
	EdgeAdder edge_adder(time_expanded_graph, weights, capacities, rev_edges, residual_capacities);
				...
				for (int direction = 0; direction < 5; direction++) {
					...
					if (isDirectionAllowed(current_location, direction) && !my_map[location]) { // location is valid as a target from current_location
						...
						if (!existVertex(time_expanded_graph, in_node_pt)) {
							...
							//test
							VertexProperties* out_node_pt = new VertexProperties{ location, -1, false, true, false, current_timestep + 1 };
							assert(existVertex(time_expanded_graph, out_node_pt));
...
/*
	void SingleTypeSearchECBS::setHashtable(int layer_time_step) {
		hashtable_t hash;
	...
	SingleTypeSearchECBS::Graph_Vertex_t SingleTypeSearchECBS::addNode(VertexProperties* vp, int layer_time_tep) {
	...
	void SingleTypeSearchECBS::updatePath(Node* goal) {
		path.clear();
		Node* curr = goal;
	...
	inline void SingleTypeSearchECBS::releaseClosedListNodes(hashtable_t* allNodes_table) {
...
inline bool SingleTypeSearchECBS::isConstrained(int curr_id, int next_id, int next_timestep, const vector< list< pair<int, int> > >* cons) {
...
bool SingleTypeSearchECBS::findPath(double focal_w, const vector < list< pair<int, int> > >* constraints, vector<vector<int>> & res_table, int cost_lowerbound) {

	// current constraints
	while (true) {	
		
		if (current_timestep < cost_lowerbound) {
		setUntillLayer(cost_lowerbound);
		}
		...
		int maxflow_value = boost::edmonds_karp_max_flow(modified_time_expanded_graph, source_node, terminal_node);
	...
	//relax timestep
	int suboptimal_bound = (int)(focal_w * cost_lowerbound);

	if (current_timestep < suboptimal_bound) {
		setUntillLayer(suboptimal_bound);
	}
	...
	updateWeights(modified_time_expanded_graph, boost::get(boost::edge_weight, modified_time_expanded_graph), res_table);
	...
	assert(maxflow_value == num_agents);
...
vector<int> SingleTypeSearchECBS::getDistancesToAllLocations(int start) {
...
void SingleTypeSearchECBS::printPaths() {
...
void SingleTypeSearchECBS::updateWeights(Graph_t & g, Weight & w, vector<vector<int>> & res_table) {

	for (int t = 0; t < res_table.size(); t++) {
		for (int location = 0; location < res_table[t].size(); location++) {
			if (res_table[t][location] > 0) {
				VertexProperties* in_node_pt = new VertexProperties{ location, -1, true, false, false, t };
				if (existVertex(g, in_node_pt)) {
					VertexProperties* out_node_pt = new VertexProperties{ location, -1, false, true, false, t };
					Graph_Vertex_t in_node = vertex_index_map[in_node_pt];
					Graph_Vertex_t out_node = vertex_index_map[out_node_pt];
					bool exst;
					Graph_Edge_t reversed_edge;
					boost::tie(reversed_edge, exst) = boost::edge(in_node, out_node, g);
					if (exst) {
						w[reversed_edge] = res_table[t][location];
					}
				}
			}
		}
...
/*
void SingleTypeSearchECBS::updateReachableSets(int timesteps) {
...
SingleTypeSearchECBS::~SingleTypeSearchECBS() {
