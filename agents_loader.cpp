...
AgentsLoader::AgentsLoader(string fname) {
	...
	if (myfile.is_open()) {
		...
		for (int t = 0; t < num_of_types; t++) {
			vector< pair<int, int> > initial_locations_sametype;
			vector< pair<int, int> > goal_locations_sametype;
			for (int i = 0; i < nums_of_agents[t]; i++) {
				...
				initial_locations_sametype.push_back(curr_pair);
				c_beg++;
				curr_pair.first = atoi((*c_beg).c_str());
				c_beg++;
				curr_pair.second = atoi((*c_beg).c_str());
				//      cout << "GOAL[" << curr_pair.first << "," << curr_pair.second << "]" << endl;
				goal_locations_sametype.push_back(curr_pair);
			}
