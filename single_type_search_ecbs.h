#ifndef SINGLETYPESEARCHECBS_H
#define SINGLETYPESEARCHECBS_H
...
public:
	...
	SingleTypeSearchECBS(vector<int> s_locs, vector<int> g_locs, const double* heuristic,
		const vector<bool> & my_m, int rows, int cols, const int* actions,
		const EgraphReader* egr = NULL, double e_w = 1.0, bool tweak = false);
	...
	bool findPath(double focal_w, const vector < list< pair<int, int> > >* constraints, vector<vector<int>> & res_table, int cost_lowerbound);

	~SingleTypeSearchECBS();
...
private:
	...
	void updateWeights(Graph_t & g, Weight & w, vector<vector<int>> & res_table);
